...
public:
    _lightweight_vector_iterator()
        : _myvec(0)
        , _where(0)
    {
    }
    _lightweight_vector_iterator(lightweight_vector<T>* const vec, SIZE_T index)
        : _myvec(vec)
        , _where(index)
    {
    }
    _lightweight_vector_iterator(_lightweight_vector_iterator const& source)
        : _myvec(source._myvec)
    ...
    T& operator*() { return (*_myvec)[_where]; }
...
public:
    ...
    lightweight_vector(lightweight_vector const& source)
        : at(source.at ? new T[source.cap] : 0)
    ...
    void erase(iterator const _where) { erase(at + _where._where); }
    ...
    void erase(SIZE_T const offset) { erase(at + offset); }
