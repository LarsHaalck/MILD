...
{
public:
    SparseMatcher(int feature_type, int input_hash_table_num, int input_depth_level,
        float input_distance_threshold);
    ~SparseMatcher();
    ...
    void search_8_with_range(cv::Mat desc, std::vector<cv::DMatch>& matches,
        ...
        std::vector<cv::KeyPoint>& query_features, float range);
