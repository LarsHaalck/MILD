...
{
public:
    BayesianFilter(float input_probability_threshold,
        int input_non_loop_closure_threshold, float input_min_shared_score_threshold,
    ...
    // output	:	salient_score, (similarity - std)/average, which reveals the probability
    // of loop closure
    inline void calculateSalientScore(
        const std::vector<float>& similarity_score, std::vector<float>& salient_score)
    {
        ...
        float delta = (sim_score.rowwise() - sim_score.colwise().mean()).norm()
            / fmax(sqrt(sim_score.size() - 1), 1);
        ...
        if (dataset_size > min_distance)
        {
            ...
            for (int i = 0; i < sim_score.size(); i++)
            {
                ...
                float alpha = previous_visit_probability
                                  ...
                                  .maxCoeff();
                float prob1 = likelihood * (trans_model[1][0]) * (1 - alpha)
                    + likelihood * trans_model[1][1] * alpha;
                float prob2
                    = non_loop_closure_threshold * (trans_model[0][0]) * (1 - alpha)
                    + non_loop_closure_threshold * trans_model[0][1] * alpha;
                current_visit_probability[i] = prob1 / (prob1 + prob2);
                current_visit_flag[i]
                    = current_visit_probability[i] > probability_threshold;
            }
            ...
            if (privious_visit_flag.size() >= 4)
            {
                ...
                for (int i = 0; i < search_range; i++)
                {
                    if (privious_visit_flag[previous_visit_idx][i] > 0)
                    {
                        ...
                        if (current_visit_flag
                                .segment(start_loop_closure,
                                    end_loop_closure - start_loop_closure)
                                .maxCoeff()
                            == 0)
                        {
                            int p2flag = privious_visit_flag[previous_visit_idx - 2]
                                             .segment(start_loop_closure,
                                                 end_loop_closure - start_loop_closure)
                                             .maxCoeff();
                            int p1flag = privious_visit_flag[previous_visit_idx - 1]
                                             .segment(start_loop_closure,
                                                 end_loop_closure - start_loop_closure)
                                             .maxCoeff();
                            if (p2flag + p1flag < 2)
                            {
                                privious_visit_flag[previous_visit_idx - 2]
                                    .segment(start_loop_closure,
                                        end_loop_closure - start_loop_closure)
                                    .setZero();
                                privious_visit_flag[previous_visit_idx - 1]
                                    .segment(start_loop_closure,
                                        end_loop_closure - start_loop_closure)
                                    .setZero();
                                privious_visit_flag[previous_visit_idx]
                                    .segment(start_loop_closure,
                                        end_loop_closure - start_loop_closure)
                                    .setZero();
                            }
