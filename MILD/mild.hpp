    ...
    int hash_table_num, int bits_per_substring)
{
    ...
    else
    {
        ...
        for (int i = 0; i < hash_table_num; i++)
        {
            ...
            if (si < ei)
            {
                index = ((desc[si] >> sp) & mask[32 - sp])
                    + ((desc[ei] & mask[ep]) << (bits_per_substring - ep));
            }
...
// output neighbor_entry_idx of entry_idx, whose hamming distance is less than
// my_depth_leve.
inline void generate_neighbor_candidates(int my_depth_level, unsigned long entry_idx,
    std::vector<unsigned long>& neighbor_entry_idx, int bits_per_substring)
{
    if (my_depth_level <= 0)
    {
        return;
    ...
    for (int i = 0; i < bits_per_substring; i++)
    {
        ...
        generate_neighbor_candidates(
            my_depth_level, nidx, neighbor_entry_idx, bits_per_substring);
    }
    return;
}
