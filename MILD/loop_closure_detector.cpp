...
LoopClosureDetector::LoopClosureDetector(int feature_type, int para_table_num,
    int input_depth_level, int input_distance_threshold, int input_max_num_per_entry)
{
    switch (feature_type)
    {
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        lut_feature_similarity[i]
            = expf(-hamming_distance * hamming_distance / HAMMING_COVARIANCE);
    }
}
...
int LoopClosureDetector::insert_and_query_database(
    cv::Mat desc, std::vector<float>& score)
{
    if (descriptor_type == FEATURE_TYPE_ORB)
    {
        int descriptor_length = desc.cols * 8;
...
void LoopClosureDetector::search_entry(
    uint64_t* f1, unsigned long search_entry_idx, std::vector<float>& score)
{
    ...
    for (int i = 0; i < conflict_num; i++)
    {
        ...
        int hamming_distance
            = (_mm_popcnt_u64(*(f1) ^ *(f2)) + _mm_popcnt_u64(*(f1 + 1) ^ *(f2 + 1))
                + _mm_popcnt_u64(*(f1 + 2) ^ *(f2 + 2))
                + _mm_popcnt_u64(
                    *(f1 + 3) ^ *(f2 + 3))); // calculate_hamming_distance_256bit(f1, f2);
