...
SparseMatcher::SparseMatcher(int feature_type, int input_hash_table_num,
    int input_depth_level, float input_distance_threshold)
{
    switch (feature_type)
    {
...
{
    cout << "num of distance calculation : " << statistics_num_distance_calculation
         << endl;
}
...
void SparseMatcher::BFMatch(cv::Mat d2, cv::Mat d1, std::vector<cv::DMatch>& matches)
{
    ...
    for (int f1 = 0; f1 < feature_f1_num; f1++)
    {
        ...
        for (int f2 = 0; f2 < feature_f2_num; f2++)
        {
            int hamming_distance
                = calculate_hamming_distance_256bit(feature1_ptr, d2.ptr<uint64_t>(f2));
    ...
    fwrite(
        delta_distribution, sizeof(unsigned short), feature_f1_num * feature_f2_num, fp);
    fclose(fp);
}
...
void SparseMatcher::search_8_with_range(cv::Mat desc, std::vector<cv::DMatch>& matches,
    std::vector<cv::KeyPoint>& train_features, std::vector<cv::KeyPoint>& query_features,
    float range)
{
    int feature_num = desc.rows;
    ...
    for (unsigned short feature_idx = 0; feature_idx < feature_num; feature_idx++)
    {
        ...
        for (int hash_table_id = 0; hash_table_id < hash_table_num; hash_table_id++)
        {
            ...
            for (int i = 0; i < conflict_num; i++)
            {
                ...
                if ((train_feature.x - query_feature.x)
...
bool less_match_id(DMatch m1, DMatch m2) { return m1.trainIdx < m2.trainIdx; }
bool less_match_score(DMatch m1, DMatch m2) { return m1.distance < m2.distance; }
bool equal_match_feature_id(DMatch m1, DMatch m2) { return m1.trainIdx == m2.trainIdx; }
void SparseMatcher::search_8(cv::Mat desc, std::vector<cv::DMatch>& matches)
{
    int feature_num = desc.rows;
    matches.clear();
    for (unsigned short feature_idx = 0; feature_idx < feature_num; feature_idx++)
    {
        ...
        feature_matches.erase(unique(feature_matches.begin(), feature_matches.end(),
                                  equal_match_feature_id),
            feature_matches.end());
        sort(feature_matches.begin(), feature_matches.end(),
            less_match_score); //  distance in ascending order
...
void SparseMatcher::match(cv::Mat d1, cv::Mat d2, std::vector<cv::DMatch> &matches)
{
        ...
        //	ippsZero_64s((Ipp64s*)features, buckets_num * max_buckets_num *
sizeof(short) / sizeof(Ipp64s));
        ...
        for (int feature_id = 0; feature_id < feature_f1_num; feature_id++)
        {
                ...
                for (int unit_id = 0; unit_id < unit_num; unit_id++)
                {
                        ...
                        {
                                int hash_table_id = unit_id * chunk_num_per_unit +
chunk_id;		//	the order of hash table int chunk_pos = (unsigned
int)((*(feautres_f1) >> (bits_per_chunk * chunk_id)) & hash_mask) + (hash_table_id)*
offset_per_hash_table; chunk_pos = chunk_pos * max_buckets_num; if
(features_buffer[chunk_pos] < max_buckets_num - 1)
                                {
                                        features_buffer[chunk_pos] += 1;
                                        features_buffer[chunk_pos +
features_buffer[chunk_pos]] = feature_id;
                                }
                        }
                        feautres_f1++;
                }
        ...
        for (int feature_id = 0; feature_id < feature_f2_num; feature_id++)
        {
                ...
                for (int unit_id = 0; unit_id < unit_num; unit_id++)
                {
                        ...
                        {
                                int hash_table_id = unit_id * chunk_num_per_unit +
chunk_id;		//	the order of hash table int chunk_pos = ((*(feautres_f2)
>> (bits_per_chunk * chunk_id)) & hash_mask)
                                        + (hash_table_id)* offset_per_hash_table;
                                int candidate_num = features_buffer[chunk_pos *
max_buckets_num]; unsigned short *candidate_pos = &features_buffer[chunk_pos *
max_buckets_num + 1];

                                for (int candidate_index = 0; candidate_index <
candidate_num; candidate_index++)
                                {
                                        int corresponding_fid = *(candidate_pos +
candidate_index);
                                        // 					if
(visited_flag[corresponding_fid] == 0)
                                        {
                                                //
visited_flag[corresponding_fid] = 1; uint64_t *feature1_ptr = (uint64_t*)d1.data +
(corresponding_fid)* 4; int hamming_distance = ORB_HammingDistance_uint64(feature1_ptr,
feature2_ptr); num_distance_calculation++; if (hamming_distance < min_distance)
                                                {
                                                        min_distance = hamming_distance;
                                ...
                                {
                                        chunk_pos = ((*(feautres_f2) >> (bits_per_chunk *
chunk_id)) & hash_mask) ^ (1 << iter)
                                                + (hash_table_id)* offset_per_hash_table;
                                        candidate_num = features_buffer[chunk_pos *
max_buckets_num]; unsigned short *candidate_pos = &features_buffer[chunk_pos *
max_buckets_num + 1]; for (int candidate_index = 0; candidate_index < candidate_num;
candidate_index++)
                                        {
                                                int corresponding_fid = *(candidate_pos +
candidate_index);
...
if (visited_flag[corresponding_fid] == 0)
                                                {
                                                        //
visited_flag[corresponding_fid] = 1; uint64_t *feature1_ptr =
d1.ptr<uint64_t>(corresponding_fid); int hamming_distance =
ORB_HammingDistance_uint64(feature1_ptr, feature2_ptr); num_distance_calculation++; if
(hamming_distance < min_distance)
                                                        {
                                                                min_distance =
hamming_distance; best_corr_fid = corresponding_fid;
                                                        }
                                                }
                                        }
                                }
